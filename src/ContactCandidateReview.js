...
import { contactRoles, webpageRoles } from './accountEvidence.js';
import { safeSourceUrl } from './associations.js';

export default {
  props:{market:{type:String,default:'HU'},group:Object,saving:Boolean,preferred:Boolean},
  emits:['review','prefer'],
  setup(props,{emit}){
    ...
    const roles=computed(()=>Object.entries(contactRoles).filter(([key])=>props.group.field==='website'?[...Object.keys(webpageRoles),...(props.market==='HU'?['historical','unrelated','unresolved']:[])].includes(key):props.group.field==='email'?![...Object.keys(webpageRoles),'fax','out_of_hours'].includes(key):!Object.hasOwn(webpageRoles,key)));
  ...
  template:`<article class="mapping-card contact-review-card" :data-contact-id="group.id">
    ...
    <p class="contact-review-meta">{{market==='HU'?'HSZ '+(group.service_ids?.join(', ')||'Service not established'):'Account association · individual doctor allocation unverified'}}<span v-if="group.independent_source_families!=null"> · {{group.independent_source_families}} distinct source families · {{group.proof_count}} assessed excerpts</span></p>
    ...
    <p v-if="group.field==='email'&&market==='HU'"><a :href="'#email='+encodeURIComponent(group.value)" target="_blank" rel="noopener noreferrer">Review email validity separately</a></p><p role="status">{{copyStatus}}</p>
  </article>`
};
