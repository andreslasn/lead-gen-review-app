export default {
  props:{email:String,index:Number,total:Number,disabled:Boolean,editable:Boolean},
  emits:['move','copy','edit'],
  template:`
    <nav class="review-navigation" aria-label="Email navigation">
      <button :disabled="index<=0||disabled" @click="$emit('move',-1)" aria-label="Previous email" title="Previous email (←)">←</button>
      <span>{{index+1}} / {{total}}</span>
      <button :disabled="index>=total-1||disabled" @click="$emit('move',1)" aria-label="Next email" title="Next email (→)">→</button>
    </nav>
    <div class="email-title-row">
      <h2>{{email}}</h2>
      <button class="copy-btn" @click="$emit('copy',email)" title="Copy email" aria-label="Copy email"><svg width="16" height="16" viewBox="0 0 16 16" fill="none"><rect x="5" y="5" width="9" height="9" rx="1.5" stroke="currentColor" stroke-width="1.5"/><path d="M11 5V3.5A1.5 1.5 0 009.5 2h-6A1.5 1.5 0 002 3.5v6A1.5 1.5 0 003.5 11H5" stroke="currentColor" stroke-width="1.5"/></svg></button>
