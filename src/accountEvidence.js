...
export function validateAccountPackage(pkg, manifest) {
  ...
  for (const a of pkg.accounts) {
    if (!/^HU:[A-Z0-9]{4}$/.test(a.account_key) || codes.has(a.account_key) || !Array.isArray(a.claims)) throw Error('Invalid account identity.');
    ...
    for (const c of a.claims) {
      ...
      if(indexed&&(!/^account-evidence\/[A-Z0-9]{4}-[a-f0-9]{16}\.json$/.test(a.evidence_path||'')||!/^[a-f0-9]{64}$/.test(a.evidence_sha256||'')))throw Error('Invalid account evidence reference.');
      if (!/^[a-f0-9]{64}$/.test(c.claim_id) || ids.has(c.claim_id) || c.account_key!==a.account_key || !researchFields[c.field] || c.review_status!=='unreviewed' || !indexed&&(!Array.isArray(c.observations) || !c.observations.length) || !a.services.some(s=>s.hsz===c.hsz)) throw Error('Invalid account claim.');
