...
export const researchFields = {website:'Website', email:'Email account link', telephone:'Telephone', practice_software:'Practice software', patient_tools:'Patient tools', opening_hours:'Opening hours',doctor_name:'Doctor at this practice',practice_name:'Practice name', patient_count_clue:'Patient count clue (scope unverified)'};
...
export function validateContactPreference(e,pkg) {
  if(!e?.id||e.schema_version!==1||(!validAccountKey(e.account_key)||e.account_key.startsWith('LV:'))||!contactFields.includes(e.field)||(e.candidate_id!==null&&!/^[a-f0-9]{64}$/.test(e.candidate_id||''))||!e.reviewed_by||!Number.isFinite(Date.parse(e.reviewed_at))||!Array.isArray(e.supersedes)||e.supersedes.some(id=>typeof id!=='string'||id===e.id))throw Error('Invalid preferred-contact decision.');
...
export function contactPresentation(account,fieldEvents=[],associations=new Map(),preferences=[],validity={}) {
  ...
  const groups=resolution.groups.map(g=>{
    const states=g.field==='email'&&account.account_key.startsWith('HU:')?[associations.get(g.association_id)||{status:g.recorded_ownership||'unreviewed'}]:g.claim_ids.map(id=>fieldState(claimMap.get(id),fieldEvents));
    const statuses=new Set(states.map(s=>s.status)),roles=new Set(states.filter(s=>s.status==='confirmed').map(s=>g.field==='email'&&account.account_key.startsWith('HU:')?s.contact_purpose:s.contact_role).filter(Boolean));
...
export function validateWebpageReviewExport(payload,pkg) {
  ...
  if(payload.contact_preferences!=null&&!Array.isArray(payload.contact_preferences))throw Error('Invalid contact preferences.');
  for(const e of payload.contact_preferences||[]){if(!e.account_key?.startsWith(pkg.country+':'))throw Error('Contact preference market mismatch.');validateContactPreference(e,pkg);}
  return payload;
}
