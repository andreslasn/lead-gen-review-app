...
(async()=>{
 ...
 try{
  ...
  await page.getByLabel('Market',{exact:true}).selectOption('LV');await page.getByLabel('Page type',{exact:true}).selectOption('all');await page.getByRole('button',{name:'All 2',exact:true}).click();
  ...
  assert.deepEqual(errors,[]);console.log('LV webpage browser checks passed: shared HU layout, arrow buttons/keys, guarded shortcuts, choice/confirmation, market isolation, saved evidence, failure/retry, decisions, reload, JSON round trip, protected HU validation/usage, legacy links and mobile layout.');
