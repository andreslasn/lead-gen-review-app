...
await context.route('**/*',async route=>{
 ...
 if(url.pathname.endsWith('/saved-doctor.txt'))return route.fulfill({status:200,contentType:'text/plain',body:JSON.stringify({format:'lead-gen-account-evidence-gzip',schema_version:1,data:gzipSync('Second Doctor\nValidated source record for Second Doctor\nnamed@example.invalid').toString('base64')})});
 if(url.pathname.includes('/data/')){const name=url.pathname.split('/').at(-1),body=JSON.stringify(payloads[name]||{});return route.fulfill({status:payloads[name]?200:404,contentType:'application/json',body:url.pathname.includes('/clinics/')?JSON.stringify({format:'lead-gen-account-evidence-gzip',schema_version:1,data:gzipSync(body).toString('base64')}):body});}
