...
const ro=()=>JSON.parse(JSON.stringify(lvPackage()).replaceAll('LV:','RO:').replaceAll('"LV"','"RO"').replaceAll('synthetic-lv','synthetic-ro').replaceAll('lv-hash','ro-hash'));
test('Romania reviews keep account links separate from HU validity and preserve market boundaries',()=>{
 const pkg=ro(),a=pkg.accounts[0],email={...a.claims[0],claim_id:'d'.repeat(64),field:'email',value:'reception@example.invalid'};
...
test('Romania website capabilities can record prescription requests without asserting software',()=>{
 const pkg=ro(),e={...lvEvent(),account_key:pkg.accounts[0].account_key,contact_role:'practice_website',webpage_capabilities:{version:1,actions:['prescription_request'],provider:''}};
